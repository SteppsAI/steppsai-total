  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 h-14">
        <div className="h-full mx-auto flex items-center justify-between px-4 sm:px-6 xl:px-8 max-w-[90rem]">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
            ...
            <span className="font-medium text-sm hidden sm:inline-block truncate">
              {guide.title}
            </span>
          </div>
      ...
      {/* Main Content */}
      <main className="flex-1 w-full mx-auto px-4 py-6 sm:px-6 sm:py-8 md:py-12 lg:py-12 xl:py-16 xl:px-8 max-w-[90rem]">
        <div className="max-w-4xl mx-auto space-y-8 sm:space-y-12 md:space-y-16">
          {/* Guide Header */}
          <div className="space-y-4 sm:space-y-6 text-center border-b pb-8 sm:pb-12">
            <div className="space-y-3 sm:space-y-4">
              <Badge variant={guide.status === 'published' ? 'default' : 'secondary'} className="uppercase tracking-wider text-[10px]">
                {guide.status || 'Draft'}
              </Badge>
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-foreground break-words px-2">
                {guide.title}
              </h1>
              {guide.description && (
                <p className="text-base sm:text-lg md:text-xl text-muted-foreground leading-relaxed max-w-2xl mx-auto break-words px-2">
                  {guide.description}
                </p>
              )}
            </div>
            <div className="flex items-center justify-center gap-3 sm:gap-4 text-xs sm:text-sm text-muted-foreground flex-wrap">
              ...
              <span className="hidden sm:inline">Last updated {new Date(guide.updatedAt || new Date()).toLocaleDateString()}</span>
            </div>
          </div>
          ...
          {steps.length > 0 ? (
            <div className="space-y-12 sm:space-y-16 md:space-y-20">
              {steps.map((step, index) => (
                <div key={step.id || index} className="space-y-4 sm:space-y-6 group">
                  <div className="flex flex-col gap-3 sm:gap-4">
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className="flex-none flex items-center justify-center size-7 sm:size-8 rounded-full bg-primary/10 text-primary font-bold text-xs sm:text-sm mt-0.5 sm:mt-1">
                        {index + 1}
                      </div>
                      <div className="space-y-2 pt-0.5 sm:pt-1 min-w-0 flex-1">
                        <h2 className="text-lg sm:text-xl md:text-2xl font-medium text-foreground leading-snug break-words">
                  ...
                  {/* Screenshot */}
                  <div className="rounded-lg sm:rounded-xl border overflow-hidden shadow-sm bg-muted/10 ring-1 ring-black/5">
                    {step.imageKey ? (
                      <img
                        src={step.imageKey}
                        alt={`Step ${index + 1}`}
                      ...
                      <div className="aspect-video flex items-center justify-center text-muted-foreground bg-muted/20">
                        <p className="text-lg sm:text-xl md:text-2xl font-medium text-center px-4 sm:px-6">
                          {step.caption || step.aiCaption || `Step ${index + 1}`}
                        </p>
                      </div>
                    )}
            ...
            <div className="flex flex-col items-center justify-center py-16 sm:py-20 text-muted-foreground border-2 border-dashed rounded-xl">
              <p className="text-base sm:text-lg">No steps in this guide.</p>
